 ...
 * theme name: Labubu
 * version: 1.0
 * description: Labubu - Meme coin ICO & Crypto Tailwindcss Template 
 * author: Pixelaxis
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  accordion.forEach((item, index) => {
    accordion[index].addEventListener("click", function () {
      ...
      accordion[index].parentElement.classList.toggle("!border-accent");
    });
  });
    ...
    item.innerHTML = words
      .map((word, wordIndex) => {
        const lettersHTML = word
          .split("")
          .map((letter) => {
            return `<span class="letter-span" style="display: inline-block; opacity: 0;">${letter}</span>`;
          })
          .join("");
